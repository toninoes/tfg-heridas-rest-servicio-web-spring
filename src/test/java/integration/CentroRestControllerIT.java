...
public class CentroRestControllerIT {
	...
	private static String URLSERVICIO	= "/api/centros/";
	...
	public void test1() { 
		...
		Centro nuevo = new Centro("AAA", "BBB");

		HttpEntity<Centro> entidad = new HttpEntity<Centro>(nuevo, cabecera);

		ResponseEntity<Centro> respuesta = restTemplate.exchange(
				crearUrlConPuerto(URLSERVICIO),
				HttpMethod.POST, entidad, Centro.class);
	...
	public void test2() {
		...
		ResponseEntity<Centro> respuesta = restTemplate.exchange(
				...
				HttpMethod.GET, entidad, Centro.class);

		assertThat(respuesta.getBody().getNombre(), equalTo("AAA"));
		assertThat(respuesta.getBody().getDireccion(), equalTo("BBB"));
	}
	...
	public void test3() { 
		...
		Centro editar = new Centro("XXX", "YYY");

		HttpEntity<Centro> entidad = new HttpEntity<Centro>(editar, cabecera);

		ResponseEntity<Centro> respuesta = restTemplate.exchange(
				crearUrlConPuerto(URLSERVICIO + ID),
				HttpMethod.PUT, entidad, Centro.class);
